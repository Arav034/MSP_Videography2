...
export default function Dropzone({
  ...
  const handleFilesSelected = async (files) => {
     ...
     */
    const accepted = fileArray.filter(
      (file) =>
    ...
    /*
     * Show error if unsupported files were selected
     */
    if (accepted.length !== fileArray.length) {
      setError(
        "Some files were not supported. Please select image, video, or audio files."
      );
    }
     ...
     * =====================================================
     *
     * If uploadNumber is NOT provided,
     * keep files in memory and send them directly
     * to UploadContext.
     */
    if (!uploadNumber) {
      onFiles(accepted);
    ...
    /*
     * Send successfully uploaded files
     */
  ...
  /*
   * Remove R2 uploaded file from local list
   */
